...
class App extends Component {
  ...
  // intialize oreId
  oreId = new OreId({ appName: 'ORE ID Sample App',
    appId,
    apiKey, oreIdUrl,
    authCallbackUrl,
    signCallbackUrl,
  ...
  async handleSignButton(permissionIndex) {
    ...
    provider = 'oreid'; // default to ore id
  ...
  async handleSignSampleTransaction(provider, account, chainAccount, chainNetwork, permission) {
    try {
      ...
      let signOptions = {
        allowChainAccountSelection: true,
...
*/
  ...
  render() {
    ...
    return (
      <div>
        ...
        <h3 style={{ color: 'green', margin: '50px' }}>{isBusy && 'working...'}</h3>
        <div style={{ color: 'red', margin: '50px' }}>{errorMessage && errorMessage}</div>
        <div style={{ color: 'blue', marginLeft: '50px', marginTop: '50px' }}>{transactionId && `Returned transactionId: ${transactionId}`}</div>
        <div style={{ color: 'blue', marginLeft: '50px', marginTop: '10px' }}>{signedTransaction && `Returned signed transaction: ${signedTransaction}`}</div>
        <div style={{ color: 'blue', marginLeft: '50px', marginTop: '10px' }}>{signState && `Returned state param: ${signState}`}</div>
  ...
  renderDiscoverOptions() {
    let chainNetwork = chainNetworkForExample;
    this.walletButtons = [{ provider: 'scatter', chainNetwork }, { provider: 'ledger', chainNetwork }, { provider: 'lynx', chainNetwork }, { provider: 'meetone', chainNetwork }, { provider: 'tokenpocket', chainNetwork }, { provider: 'portis', chainNetwork }, { provider: 'whalevault', chainNetwork }, { provider: 'simpleos', chainNetwork }, { provider: 'keycat', chainNetwork }];
    return (
      <div>
        <div style={{ marginTop: 50, marginLeft: 20 }}>
  ...
  renderLoginButtons() {
    return (
      <div>
        ...
        />
        <LoginButton provider='ledger' buttonStyle={{ width: 250, marginTop: '24px' }} logoStyle={{ marginLeft: 0 }} onClick={() => this.handleLogin('ledger')} />
        <LoginButton provider='meetone' buttonStyle={{ width: 250, marginTop: '24px' }} logoStyle={{ marginLeft: 0 }} onClick={() => this.handleLogin('meetone')} />
        <LoginButton provider='lynx' buttonStyle={{ width: 250, marginTop: '24px' }} logoStyle={{ marginLeft: 0 }} onClick={() => this.handleLogin('lynx')} />
        <LoginButton provider='portis' buttonStyle={{ width: 250, marginTop: '24px' }} logoStyle={{ marginLeft: 0 }} onClick={() => this.handleLogin('portis')} />
        <LoginButton provider='whalevault' buttonStyle={{ width: 250, marginTop: '24px' }} logoStyle={{ marginLeft: 0 }} onClick={() => this.handleLogin('whalevault')} />
        <LoginButton provider='simpleos' buttonStyle={{ width: 250, marginTop: '24px' }} logoStyle={{ marginLeft: 0 }} onClick={() => this.handleLogin('simpleos')} />
        <LoginButton provider='keycat' buttonStyle={{ width: 250, marginTop: '24px' }} logoStyle={{ marginLeft: 0 }} onClick={() => this.handleLogin('keycat')} />
      </div>
    );
  }
}
